      ...
      <v-layout
        ...
        <v-flex
          ...
          <v-card-media
            ...
            :height="$vuetify.breakpoint.smAndDown ? '300px' : '377px'"
          ...
          <div>
            ...
            <v-list two-line>
              <v-list-tile
                ...
                :to="`/${parent.id}`">
                <v-list-tile-content>
                  <v-list-tile-sub-title>Основная история:</v-list-tile-sub-title>
                  <v-list-tile-title class="accent--text">{{ parent.russian || parent.name }}</v-list-tile-title>
                </v-list-tile-content>
              </v-list-tile>
              ...
              <v-list-tile
                class="blue--text blue lighten-5"
                v-else-if="anime.ongoing">
                <v-list-tile-action>
              ...
              <v-list-tile
                v-if="anime.score !== '0.0'"
                :class="ratingColor ? `${ratingColor} ${ratingColor}--text lighten-5` : ''">
                <v-list-tile-action>
            ...
            <v-list v-if="childs && childs.length">
              <v-subheader>Ответвлени{{ childs.length === 1 ? 'е' : 'я' }}</v-subheader>
              <anime-card-related
            ...
            <v-card-actions>
              ...
              <v-spacer/>
              <v-btn
                :href="`https://play.shikimori.org/animes/${anime.id}`"
                target="_blank"
                color="accent">
                Смотреть онлайн
              </v-btn>
...
export default {
	name: 'SingleAnimeCard',
	components: {animeCardRelated},
	mixins: [hasRate, franchise],
	props: {
		anime: {
			type: Object,
			required: true,
